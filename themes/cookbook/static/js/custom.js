...
var fuseOptions = {
  ...
  keys: [
    ...
    {name:"ingredientes",weight:0.3},
    {name:"categorias",weight:0.3}
  ]
};
    ...
    Object.entries(result).forEach(entry => {
        ...
        if( fuseOptions.tokenize ){
          ...
          value.matches.forEach(function(matchKey,mvalue){
            ...
            if(mvalue.key == "ingredientes" || mvalue.key == "categorias" ){
        ...
        if(snippet.length<1){
          ...
          //replace values
          var output = render(templateDefinition,{key:key,title:value.item.title,link:value.item.permalink,ingredientes:value.item.ingredientes,categorias:value.item.categorias,snippet:snippet,image:value.item.imageLink});
